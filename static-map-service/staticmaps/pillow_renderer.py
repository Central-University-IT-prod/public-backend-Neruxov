...
class PillowRenderer(Renderer):
    ...
    def render_tiles_threaded(self, download: typing.Callable[[int, int, int], typing.Optional[bytes]]) -> None:
        """Render background of static map using multi-threading.

        :param download: Callable function to download a tile.
        :type download: typing.Callable[[int, int, int], typing.Optional[bytes]]
        """
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for yy in range(0, self._trans.tiles_y()):
                ...
                for xx in range(0, self._trans.tiles_x()):
                    x = (self._trans.first_tile_x() + xx) % self._trans.number_of_tiles()
                    futures.append(executor.submit(task, x, y))

            for future in concurrent.futures.as_completed(futures):
                result = future.result()
                if result is not None:
                    tile_img, x, y = result
                    xx = x - self._trans.first_tile_x()
                    yy = y - self._trans.first_tile_y()
                    self._image.paste(
                        tile_img,
                        (
                            int(xx * self._trans.tile_size() + self._trans.tile_offset_x()),
